...
() {
  ...
  # Main prompt function - runs synchronously, triggers async worker
  function prompt_my_jj() {
    local workspace

    command -v jj > /dev/null 2>&1 || return
    if workspace=$(jj workspace root 2>/dev/null); then
      p10k display "*/my_jj=show"
      p10k display "*/vcs=hide"
    else
      p10k display "*/my_jj=hide"
  ...
  # Async worker function - does the heavy lifting
  function _my_jj_async() {
    local workspace=$1
    local display revision bookmark distance file_status
    ...
    bookmark=$(jj log --repository "$workspace" --ignore-working-copy \
      --no-graph --limit 1 --color always \
      -r "prompt_bookmark(@)" -T 'bookmarks.join(" ")' 2>/dev/null)
...
# Initialize async worker (must be outside the anonymous function)
